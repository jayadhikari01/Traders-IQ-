...
module.exports = async (req, res) => {
    try {
        ...
        // 3. Getting Raw Body for Signature Verification
        const chunks = [];
        ...
        // 4. Verify Lemon Squeezy Signature
        ...
        // 5. Parse Data and Update Firebase
        ...
        if (eventName === 'order_created' || eventName === 'subscription_created') {
            if (userId) {
                // This is the core logic: Updating the user in your Firestore
                await db.collection('users').doc(userId).update({
                    isPro: true,
                    plan: payload.data.attributes.variant_name || 'Premium',
                    updatedAt: admin.firestore.FieldValue.serverTimestamp()
                });
                return res.status(200).send('User Updated to Pro');
            }
        }
